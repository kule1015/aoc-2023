...
def _is_game_valid(allowed_cubes: dict[str, int], game_information: str):
    ...
    return all([_is_game_part_possible(_parse_cubes_of_game_part(e), allowed_cubes) for e in _parse_cube_sets(game_information)])


def _is_game_part_possible(cubes_of_game_part: [str, int], allowed_cubes_of_games: [str, int]) -> bool:
    return all([cubes_of_game_part[cube] <= allowed_cubes_of_games[cube] for cube in cubes_of_game_part.keys()])
